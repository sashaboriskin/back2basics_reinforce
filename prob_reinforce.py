...
def main(config_path="config.yaml", reward_model_path=None):
    ...
    if reward_model_path:
        cfg.prob_reward_model.output_dir = reward_model_path

    wandb.init(
        project=cfg.project_name,
        name=cfg.prob_reinforce.wandb_run_name,
        config={
    ...
    reward_model = AutoModelForSequenceClassification.from_pretrained(cfg.prob_reward_model.output_dir, num_labels=10).to(device)
    ...
    def compute_rewards(prompts: List[str], responses: List[str]) -> torch.Tensor:
        ...
        logits = torch.cat(logits_list, dim=0)  # [batch_size, 10]
        probs = F.softmax(logits, dim=-1)       # [batch_size, 10]
        scores = torch.arange(1, 11, device=device, dtype=probs.dtype)
        expected = (probs * scores).sum(dim=1)  # [batch_size]
        return expected

    print("Training Prob Reinforce...")
    baseline = 0.0
    os.makedirs(cfg.prob_reinforce.output_dir, exist_ok=True)

    for iteration in tqdm(range(cfg.reinforce.num_iterations)):
        batch_idx = random.sample(range(len(train_dataset)), cfg.reinforce.batch_size)
        batch = train_dataset.select(batch_idx)
        prompts = batch["prompt"]
        responses = generate_responses(prompts, cfg.reinforce.temperature)
        log_probs = compute_log_probs(prompts, responses)
        rewards = compute_rewards(prompts, responses)
        advantages = rewards - baseline
        baseline = cfg.reinforce.gamma * baseline + (1 - cfg.reinforce.gamma) * rewards.mean().item()
        ...
        if (iteration + 1) % cfg.reinforce.validation_interval == 0:
            ...
            with torch.no_grad():
                for i in range(0, min(len(eval_dataset), 100), cfg.reinforce.batch_size):
                    batch_val = eval_dataset[i:i+cfg.reinforce.batch_size]
                    prompts_val = batch_val["prompt"]
                    responses_val = generate_responses(prompts_val, cfg.reinforce.temperature)
                    rewards_val = compute_rewards(prompts_val, responses_val)
                    val_rewards.extend(rewards_val.detach().cpu().numpy())

            val_reward = np.mean(val_rewards)
            print(f"Iteration {iteration+1}, Validation reward: {val_reward:.4f}")
            wandb.log({"val_reward": val_reward, "iteration": iteration+1})

            checkpoint_dir = os.path.join(cfg.prob_reinforce.output_dir, f"checkpoint-{iteration+1}")
    ...
    sft_model.save_pretrained(cfg.prob_reinforce.output_dir)
    sft_tokenizer.save_pretrained(cfg.prob_reinforce.output_dir)
    wandb.finish()
